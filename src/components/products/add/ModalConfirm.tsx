...
import { createProducts } from "@/lib/redux/api/products";
...
export default function ModalConfirmAddProduct({ isOpen, onClose, file }: DetailChallengeModalProps) {
  ...
  const onSubmit = async () => {
    ...
    setLoading(true);
    try {
      let imageUrl: string[] = singleData.image_url || [];

      if (file && Array.isArray(file) && file.length > 0) {
        const imgUploadPromises = file.map((f) => uploadImage(f));
        const imgUpload = await Promise.all(imgUploadPromises);

        imageUrl = imgUpload.filter(Boolean) as string[];
      }

      const response = await dispatch(createProducts({
        ...singleData,
        image_url: imageUrl,
      ...
      if (response.meta.requestStatus === "fulfilled") {
        ...
        toast({
          icon: <CheckCircle />,
          variant: "default",
          description: "Berhasil menambahkan product baru!",
        });
        ...
        toast({
          icon: <CrossCircle />,
          variant: "default",
          description: "Products gagal ditambahkan!",
        });
      }
      ...
      toast({
        icon: <CrossCircle />,
        variant: "default",
        description: "Product gagal ditambahkan!",
      });
    } finally {
      setLoading(false);
    }
