...
import { FilterDataImpactOrderSchema } from "@/lib/zod/data-impact";
...
interface DataImpactOrderFilterProps {
  onFilter: (data: z.infer<typeof FilterDataImpactOrderSchema>) => void;
}

export default function DataImpactOrderFilter({ onFilter }: DataImpactOrderFilterProps) {
  const [toggleOpen, setToggleOpen] = useState<boolean>(false);

  const onToggle = () => setToggleOpen(!toggleOpen);
  const onClose = () => setToggleOpen(false);

  const form = useForm<z.infer<typeof FilterDataImpactOrderSchema>>({
    resolver: zodResolver(FilterDataImpactOrderSchema)
  });

  const onSubmit = (data: z.infer<typeof FilterDataImpactOrderSchema>) => {
    onFilter(data);
    onClose();
  }
